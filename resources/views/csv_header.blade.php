    ...
    <script>
        ...
        table.on('tool(csv_header_file_table_filter)', function (obj) {
            ...
            if (obj.event === 'delete') {
                ...
                window.location.href = "{{route('downloadCSVFile')}}" + '/' + obj.data.id+'?once=0&is_head=1';
            } else if (obj.event === 'downloadOnce'){
                window.location.href = "{{route('downloadCSVFile')}}" + '/' + obj.data.id+'?once=1&is_head=1';
            }
        });
        ...
        $('#add').on('click', function () {
            ...
            $.get('{{asset('form/add-header-file.html')}}', null, function(form) {
        ...
        table.on('edit(csv_header_file_table_filter)', function(obj){ //注：edit是固定事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            //请求服务器新增记录
            $.get("{{route('CSVHeaderManageRename')}}"+"?id="+contentData.id+'&file_name='+contentData.folder_name, function (data, status){
                if(data.code == 0){
                    layer.msg(data.msg);
                }else if(data.code == 1){
                    table.reload('csv_header_file_table_id',{
